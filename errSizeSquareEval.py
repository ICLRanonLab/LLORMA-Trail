...
#MEAN
    ...
    #PQ Analyzer
    pNAME = './llorma_g/ptmatrix-err-' + str(size)+'-'+str(mean)+'-'+str(error)+'-p.npy'
    qNAME = './llorma_g/ptmatrix-err-' + str(size) + '-' + str(mean) + '-' + str(error) + '-q.npy'
    p = np.load(pNAME)
    q = np.load(qNAME)
    ...
    csv1 = str(testType) + str(size) + '-' + str(mean) + 'TPTN' + '.csv'
    res1 = list([testType, size, mean, error, confuMat[0, 0], confuMat[1, 1]])
